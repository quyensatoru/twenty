...
// Runs after sync-issue-merchant-standard-object, which guarantees the
// issueMerchant object/fields already exist. Reuses the same generic
// searchFieldMetadata backfill built for upgrade:2-26's Epic rollout — see
// that command's own doc comment for why an empty search-fields entry would
// otherwise leave the object's searchVector permanently NULL.
@RegisteredWorkspaceCommand('2.28.0', 1784930003500)
@Command({
  name: 'upgrade:2-28:backfill-issue-merchant-search-field-metadata',
  description:
    'Backfill the missing searchFieldMetadata row for issueMerchant.id and rebuild the issueMerchant.searchVector column so existing rows stop being NULL. Idempotent.',
})
export class BackfillIssueMerchantSearchFieldMetadataCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    // The migration runner only invalidates the flat-maps keys a migration
    // touched, so sync-issue-merchant-standard-object can leave these stale.
    // The dedupe below compares live (objectMetadataId, fieldMetadataId)
    // pairs, so recompute first.
    ...
    // Creating the rows above only fixes future provisioning/introspection.
    // The searchVector column already sitting in this workspace's schema is
    // GENERATED ALWAYS AS (to_tsvector('simple', NULL)) STORED until a row
    // exists to derive it from, so only dropping and re-adding the column
    // (rebuildSearchVector) recomputes the expression and repopulates rows.
    await this.workspaceCacheService.invalidateAndRecompute(workspaceId, [
      'flatObjectMetadataMaps',
      'flatFieldMetadataMaps',
      'flatSearchFieldMetadataMaps',
