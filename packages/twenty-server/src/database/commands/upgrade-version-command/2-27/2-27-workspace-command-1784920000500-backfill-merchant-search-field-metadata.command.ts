...
// Mirrors upgrade:2-26:backfill-epic-search-field-metadata: without a search
// fields row, merchant.searchVector is GENERATED ALWAYS AS
// (to_tsvector('simple', NULL)) STORED, so every row's tsvector is NULL and
// the search resolver's `searchVector IS NOT NULL` filter makes merchant
// always return zero results from the relation-picker/global search. Runs
// after sync-merchant-standard-objects, which guarantees the merchant
// object/fields already exist.
@RegisteredWorkspaceCommand('2.27.0', 1784920000500)
@Command({
  name: 'upgrade:2-27:backfill-merchant-search-field-metadata',
  description:
    'Backfill the missing searchFieldMetadata row for merchant.name and rebuild the merchant.searchVector column so existing rows stop being NULL. Idempotent.',
})
export class BackfillMerchantSearchFieldMetadataCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
