...
const APP_SCOPE_OPERATION_BY_OPERATION_TYPE: Record<
  OperationType,
  AppScopeOperation
> = {
  select: 'read',
...
const mapOperationTypeToAppScopeOperation = (
  operationType: OperationType,
): AppScopeOperation => APP_SCOPE_OPERATION_BY_OPERATION_TYPE[operationType];
...
export class WorkspaceRepository<TEntity extends ObjectLiteral = ObjectRecord> {
  ...
  async runInsert({
    ...
    this.validateRecordVisibilityPolicyForWrittenRecords(
      this.formatResult<ObjectRecord[]>(formattedRecords),
    );
    this.validateAppScopeForWrittenRecords({
  ...
  async runBatchUpdate({
    ...
    for (const [index, input] of writableInputs.entries()) {
      ...
      this.validateRecordVisibilityPolicyForWrittenRecords(
        ...
        'Updated record does not satisfy Record Visibility Policy constraints of your current role',
      );
      this.validateAppScopeForWrittenRecords({
        values: [setColumns as Partial<ObjectRecord>],
        mode: 'update',
      });
  ...
  private validateRecordVisibilityPolicyForWrittenRecords(
    records: ObjectRecord[],
    errorMessage?: string,
  ): void {
  ...
  // Only the values being written are validated, never the merged "record
  // after update": the pre-existing row is fetched with permission checks
  // bypassed, so validating it would reject a bulk update over rows the WHERE
  // clause already excludes.
  private validateAppScopeForWrittenRecords({
    values,
    mode,
  }: {
    values: Partial<ObjectRecord>[];
    mode: 'insert' | 'update';
  }): void {
  ...
  private applyRowLevelPermissionPredicates(
    ...
    // Record Visibility Policy and App Scope only ever narrow the main
    // object's own rows — unlike row-level permissions, neither has a
    // per-join-alias equivalent.
    ...
    applyAppScopeFilter({
      ...
      authContext: this.options.authContext,
      shouldBypassPermissionChecks: this.options.shouldBypassPermissionChecks,
      operation: mapOperationTypeToAppScopeOperation(operationType),
    });
